...
class Headers extends \Fastest\Core\Init
{
    ...
    private function _csp($reportOnly = false, $nonce = '')
    {
        ...
        $_csp = [
            ...
            'style-src' => ["'self'", "'unsafe-inline'", "'unsafe-eval'"],
            'report-uri' => [sprintf('%s/policies/csp.php', $this->host)],
            'worker-src' => ["'self'"]
        ];
    ...
    {
        $this->_add('P3P: policyref="/policies/p3p.xml", CP="CAO DSP COR CURa ADMa DEVa PSAa PSDa IVAi IVDi CONi OUR OTRi IND PHY ONL UNI FIN COM NAV INT DEM STA"');
    }

    private function _expectCT()
    {
        $this->_add('Expect-CT: enforce; max-age='.$this->oneMonth.'; report-uri="'.$this->host.'/policies/expect-ct.php"');
    }
